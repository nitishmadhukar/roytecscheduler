...
//                }
    ...
    */
    ...
    setupPartnerTimelinePanel : function () {
         ...
         */
        otherPanel.on('beforezoomchange', function () {
            var scrollable = ownView.getScrollable();
            var old = scrollable.suspendPartnerSync;
            scrollable.suspendPartnerSync = Ext.emptyFn;

            // fired by buffered function after scroll end, so happen only once in the end, asyc.
            scrollable.on('scrollend', function () {
                scrollable.suspendPartnerSync = old;
            }, ownView, { single : true });
        });

        this.on('beforezoomchange', function () {
            var scrollable = otherView.getScrollable();
            var old = scrollable.suspendPartnerSync;
            scrollable.suspendPartnerSync = Ext.emptyFn;
